...
| `rules[].variants` | array\|null | нет | A/B-сплит: 2–20 взвешенных целей `{ url, weight, label? }`. Без `variants` правило ведёт на `rules[].url`. См. §7.1. |
...
- **Веса** — целые ≥ 1; доля трафика варианта = `weight / сумма весов`
  (в примере B получает 75 %). В сумму 100 давать не обязательно.
...
засчитывать ли такой переход, остаётся за вашей системой.
