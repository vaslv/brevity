...
**Статус сейчас:** все Critical и Major закрыты — C1 ✅, M2 ✅, M3 ✅ (IP-pinning опц.
открыт), M5 ✅, M6 ✅ (by design — инвариант задокументирован), M7 ✅. **0 Critical,
0 открытых Major.**

**Остаток:** Minor-пункты (m1–m14) и Documentation mismatches (D1–D6) — по желанию.
...
> **Открыто (опционально):** пиннинг резолвнутого IP против TOCTOU/DNS-rebinding —
> hardening, под админский `callback_url` некритично.
...
- **D1. `ARCHITECTURE.md` (модель данных):** «`links` — soft deletes (trashed ссылки всё
  равно резолвятся в route binding'е `ResolveLink`)» — **неверно**. `Link::findByCode()`
  применяет глобальный scope SoftDeletes → удалённые ссылки дают 404 на резолве. (В
  Filament `LinkResource::getRecordRouteBindingEloquentQuery()` действительно включает
  trashed — но это только админский просмотр.) **needs decision:** какое поведение
