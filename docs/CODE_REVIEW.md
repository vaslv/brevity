...
> **✅ Сделано в этой итерации:** m1 (убрана мёртвая `Callback::link`), m2
> (`url:http,https` + тест), m5 (валидация уникального `priority` + тесты), m8
> (`email_verified_at` в `$fillable` + тест), m11 (убран мёртвый 5-й шаг backoff +
> тест). Остальные (m3, m4, m6, m7, m9, m10, m12, m13, m14) — открыты.
