...
остаётся **0 Critical, 3 Major** (M5, M6, M7).

**С чего начать (остаток):** (1) ролевая модель или зафиксировать инвариант (**M6**);
(2) проверить/ограничить корневой `{code}` (**M5**); (3) индекс `clicks(created_at)`
(**M7**).
