...
**Статус сейчас:** C1 ✅ и M2 ✅ исправлены → остаётся **0 Critical, 4 Major**
(M3, M5, M6, M7).

**С чего начать (остаток):** (1) follow-redirect в колбеках (**M3**); (2) ролевая
модель или зафиксировать инвариант (**M6**); (3) проверить/ограничить корневой
`{code}` (**M5**); (4) индекс `clicks(created_at)` (**M7**).
