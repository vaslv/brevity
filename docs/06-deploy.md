...
**Этап 4 (геолокация)** — миграция безопасна вне окна: `clicks.geo_location_id`
добавляется **nullable** (метаданные, без переписи таблицы), а его индекс
**частичный** (`WHERE geo_location_id IS NOT NULL`) и на изначально пустом
наборе строится мгновенно; FK-валидация тривиальна (все значения NULL). После
миграции:
